...
export default function IeqTab({
  ...
  // Generate and download IEQ monitoring PDF report (filtered)
  const handleDownloadReport = () => {
    const doc = new jsPDF();
    let y = 20;
    const reportDate = new Date().toISOString().split('T')[0];
    const samples = filteredSamples; // use current filtered set
    const passCount = samples.filter(s => s.status === 'pass').length;
    const actionCount = samples.filter(s => s.status === 'action_required').length;
    const failCount = samples.filter(s => s.status === 'fail').length;

    // Period descriptor
    const monthLabel = filterMonth ? MONTH_NAMES_SHORT[parseInt(filterMonth, 10) - 1] : '';
    const periodDesc = [filterYear, monthLabel].filter(Boolean).join(' ') || 'All Time';
    ...
    doc.text(`Generated: ${reportDate}  |  Period: ${periodDesc}  |  By: ${currentUser.name}`, 14, y);
    y += 12;
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(`Total Samples: ${samples.length}`, 14, y);
    y += 6;
    ...
    // Table rows
    doc.setFont('helvetica', 'normal');
    const sortedSamples = [...samples].sort((a, b) => b.date.localeCompare(a.date));
    ...
    // Notes section
    const samplesWithNotes = samples.filter(s => s.notes);
